...
export default async function HostelPortalProfilePage() {
  const session = await getServerSession();
  if (!session) return null;

  if (!(await canAccessServerPath(session, "/portals/hostel/profile"))) {
    return <AccessDenied backHref={getDefaultPathForRole(session.role)} />;
  }

  return (
    <div className="space-y-6">
      <SupportPortalPageHeader eyebrow="My profile" title={session.name} description="Your boarding operations identity for room allocations, hostel workflow, and warden-side reporting." />
      <SupportInfoCard title="Account" description="Role and email context for this hostel session.">
