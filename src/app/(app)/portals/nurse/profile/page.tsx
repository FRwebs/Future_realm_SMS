...
export default async function NursePortalProfilePage() {
  const session = await getServerSession();
  if (!session) return null;

  if (!(await canAccessServerPath(session, "/portals/nurse/profile"))) {
  ...
  return (
    <div className="space-y-6">
      <SupportPortalPageHeader
        eyebrow="My profile"
        title={session.name}
        description="Your nurse portal identity, used across clinic history, care notes, and school health operations."
      />
      <SupportInfoCard title="Account" description="Signed-in identity and role context.">
