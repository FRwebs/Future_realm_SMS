  ...
  return (
    <div className="grid gap-6">
      <section className="rounded-[2rem] border border-white/50 bg-white/90 p-6 shadow-panel">
        ...
        <h1 className="mt-3 font-[var(--font-heading)] text-4xl font-bold text-ink">Child results</h1>
        <p className="mt-3 text-sm leading-6 text-ink/68">Only published result sheets are visible to parents.</p>
      </section>
      {results.map((result) => (
        <section key={result.id} className="rounded-[2rem] border border-white/50 bg-white/90 p-6 shadow-panel">
          <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
            <div>
              <h2 className="font-[var(--font-heading)] text-2xl font-bold text-ink">{result.session} · {result.term}</h2>
